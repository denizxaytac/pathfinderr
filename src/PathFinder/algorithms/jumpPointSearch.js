...
function addToVisit(grid, xPos, yPos, xPos2, yPos2){
    ...
    if (paths[currNode.row][currNode.col] === undefined){
        paths[currNode.row][currNode.col] = prevNode;
    }
    if (paths[nextNode.row][nextNode.col] === undefined){
...
export default function jumpPointSearch(grid, startPos, finishPos){
    ...
    while (!toVisit.isEmpty()){
        ...
        var lengthToVisit = toVisit.length();
        stepIdx = 0;
        while (stepIdx < 8){
            if (steps[stepIdx][0] === 0 || steps[stepIdx][1] === 0)
                straightPrune(grid, currNode, steps[stepIdx][0], steps[stepIdx][1]);
            else
                diagonalPrune(grid, currNode, steps[stepIdx][0], steps[stepIdx][1]);

            if (toVisit.length() !== lengthToVisit)
                break;
            stepIdx += 1;
        }
        if (stepIdx === 8)
...
function diagonalPrune(grid, startNode, stepX, stepY){
    ...
    while (true){
        ...
        if (grid[newRow + stepX]?.[newCol]?.nodeType === "wall" && grid[newRow + stepX]?.[newCol + stepY]?.nodeType !== "wall"){
            ...
            var verticalPrune = straightPrune(grid, grid[newRow][newCol], stepX, 0);
            if (!verticalPrune && cardinalForcedNeighborCheck(grid, currNode, stepX, 0)){
                addToVisit(grid, startNode.row, startNode.col, currNode.row, currNode.col);
            }
        }
        if (grid[newRow]?.[newCol + stepY]?.nodeType === "wall" && grid[newRow + stepX]?.[newCol + stepY]?.nodeType !== "wall"){
            ...
            var horizontalPrune = straightPrune(grid, grid[newRow][newCol], 0, stepY);
            if (!horizontalPrune && cardinalForcedNeighborCheck(grid, currNode, 0, stepY)){
                addToVisit(grid, startNode.row, startNode.col, currNode.row, currNode.col);
